...
export default function GroupFormInput({ formType, currentGroup }) {
  ...
  const submition = async (e) => {
    ...
    history(`/groups/${finalGroup.id}`);
  };

  useEffect(() => {
    const newErrors = {};
    if (!cityState.includes(","))
      newErrors.location = "need both city and state";
  ...
  useEffect(() => {
    if (formType === "Update") {
      ...
      setImageUrl(
        currentGroup.previewImage ?? "https://i.imgur.com/2EGj2Rk.jpeg"
      );
    }
  }, []);
  ...
  return (
    <form onSubmit={submition}>
      <div className="create-form-main">
        ...
        <hr />
        <button className="submiting-create-group-button">
          {formType} group
        </button>
      </div>
    </form>

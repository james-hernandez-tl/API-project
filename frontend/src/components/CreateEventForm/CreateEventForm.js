...
export default function CreateEventForm() {
  ...
  useEffect(() => {
    ...
    if (privateOrPublic !== "Private" && privateOrPublic !== "Public")
    ...
    if (describe.length < 50)
      newErrors.describe = "Description must be at least 50 characters long";

    if (eventStart.length && eventEnd.length) {
      if (!Date.parse(eventEnd))
        newErrors.endNotDate =
          "event end needs to be a real date formated like in the example";
      if (!Date.parse(eventStart))
        newErrors.startNotDate =
          "event start needs to be a real date formated like in the example";
      if (Date.parse(eventEnd) && Date.parse(eventStart)) {
        if (eventEnd < eventStart)
          newErrors.orderDate = "start date must be before end date";
      }
    }
  ...
  return (
    <form className="createEventForm-main" onSubmit={submitform}>
      ...
      <div className="createEventForm-input-with-icon-holder-dollar">
        <div className="createEventForm-dollar-icon-holder">
      ...
      <textarea
        ...
        value={describe}
        onChange={describeChanger}
        cols="30"
        rows="10"
      ></textarea>
      <div className="createEvent-erros">{submited && errors.describe}</div>

      <button className="CreateEvent-submit-button" type="submit">
        Create Event
      </button>
    </form>
  );
